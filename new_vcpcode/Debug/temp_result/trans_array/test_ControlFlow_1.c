...
empty int  main()
{
  ...
  while(num_type<5)
  {
    ...
    while(type_i<num_type)
    {
      ...
      while(i<7)
      {
        method_code = put_method[2*i+0];
        block_list[3*block_i] = tri_side[method_code];
        block_list[3*block_i+1] = tri_side[method_code];
        block_list[3*block_i+2] = tri_side[method_code];
        i = i+1;
        block_i = block_i+1;
      }
      type_i = type_i+1;
    ...
    while(input_block_i<block_num)
    {
      ...
      while(placed_i<input_block_i)
      {
        ...
        if(larger_result==1)
        {
          if(block_list[3*placed_i+2]>now_max_height)
          {
            now_max_height = block_list[3*placed_i+2];
          }
        }
        placed_i = placed_i+1;
      }
      block_list[3*input_block_i+2] = block_list[3*input_block_i+2]+now_max_height;
    ...
    {
      if(block_list[3*block_i+2]>now_max_height)
      {
        now_max_height = block_list[3*block_i+2];
      }
      block_i = block_i+1;
    }
    num_type = num_type+1;
...
empty int  larger(int i1,int i2)
{
  ...
  if((block_list[3*i1]>block_list[3*i2]&&block_list[3*i1+1]>block_list[3*i2+1])||(block_list[3*i1]>block_list[3*i2+1]&&block_list[3*i1+1]>block_list[3*i2]))
  {
    iRet = 1;
  }
  else
...
empty int  cmp(int i1,int i2)
{
  ...
  if(block_list[3*i1]<block_list[3*i2])
  {
    iRet = -1;
  }
  else  if(block_list[3*i1]==block_list[3*i2])
  {
    if(block_list[3*i1+1]<block_list[3*i2+1])
    {
      iRet = -1;
    }
    else
